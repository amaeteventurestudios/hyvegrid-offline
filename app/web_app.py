"""Local English web UI skeleton for HyveGrid Offline (ADTC 2026).

A minimal FastAPI app serving two screens at localhost:

- Mission Control (/): overview and navigation cards.
- Offline System Status (/status): runtime, model, retrieval, compliance, and
  benchmark evidence.

This is an English-only skeleton. It serves localhost only, uses no cloud
services, and does not load the GGUF model at import or request time. The status
values are drawn from the completed profiler/runtime evidence (Tasks 014-018).
